# TODO (MANUAL by USER)
- [x] **Re-seed Supabase for the 7 new rooms:** SQL Editor → run `supabase/seed.sql` again. Until you do, the game shows only the 3 rooms that are in the database.
...
- [x] **Vercel:** import the repo (the framework is detected as Next.js) and add the same two env vars for Production and Preview.
- [x] Check it's live: open the ❓ panel in the game. It should say "Content: Supabase".
- [x] (Optional) `git init` and commit. The project isn't a git repo yet.

# TODO (CLAUDE)

# DONE
- Added rooms 4–10 (2026-09-22): café (stem-changers), train station (-j-/-uv- stems), classroom (saber/conocer/querer/poder meaning changes), workshop (poner/dar/ver/hacer), storm night (scene vs. events), birthday party (había vs. hubo), farewell (everything). Shared objects moved to `objects/common.tsx`; `npm run playtest` plays all 10 rooms headlessly.
